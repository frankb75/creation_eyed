...
<link rel="stylesheet" type="text/css" href="<?php echo themes_url('css/uploadify.css'); ?>">
	...
	<div style="background:#c6c7cb; overflow:hidden; padding-top:23px;">
		...
		<a class="selected-menu" href="<?php echo base_url('admin/zareks_event'); ?>">Events</a>
		...
		<div style="padding:0 20px 0 40px;">
			...
			<form method="POST" enctype="multipart/form-data">
				...
				<div style="overflow:hidden;">
					...
					<img src="<?php echo base_url('uploads/gallery/zareks/'.$zareks_id.'/thumbnail/'.$thumbnail_image); ?>" style="float:left; margin-right:10px;"/>
				...
				<tr>
					...
					<td style="text-align:center;">
						...
						<form method="POST">
							...
							<?php $i = 1; $ii = 0; while($get = $this->QModel->g($query)): ?>

							<?php if($i == 6): $i = 1; ?>

							<div id="drop<?php echo $get['zareks_photos_id']; ?>|getMsg<?php echo $get['zareks_photos_id']; ?>" style="float:left; margin-bottom:5px; text-align:left;" ondrop="drop(event)" ondragover="allowDrop(event)">

								<input type="checkbox" name="deleteImg[]" value="<?php echo $get['zareks_photos_id']; ?>"/>
								...
								<div style="height:126px; width:126px; text-align:center; background:black; border:3px solid #a4a4a4; border-radius:5px; text-align:center; overflow:hidden;">
									...
									<div style="height:126px; width:126px; display:table-cell; vertical-align:middle;">
										...
										<a id="a|drag<?php echo $get['zareks_photos_id']; ?>|getMsg<?php echo $get['zareks_photos_id']; ?>" draggable="true" ondragstart="drag(event)" href="<?php echo base_url('uploads/gallery/zareks/original/'.$get['photos']); ?>" rel="lightbox[jnk]"><img id="drag<?php echo $get['zareks_photos_id']; ?>|getMsg<?php echo $get['zareks_photos_id']; ?>" src="<?php echo base_url('uploads/gallery/zareks/'.$zareks_id.'/thumbnail/'.$get['thumbnail_image']); ?>" style="margin:0 auto;"/></a>

									</div>

								</div>

								<div id="getMsg<?php echo $get['zareks_photos_id']; ?>" style="font-size:12px;"><?php echo $get['photos']; ?></div>

							</div>

							<?php else: $i++; ?>

							<div id="drop<?php echo $get['zareks_photos_id']; ?>|getMsg<?php echo $get['zareks_photos_id']; ?>" style="float:left; margin-right:16px; margin-bottom:5px; text-align:left;" ondrop="drop(event)" ondragover="allowDrop(event)">
...
function drop(event) {
	...
	xmlhttp.open("GET","<?php echo base_url('admin/dragAndDrop/'.$zareks_id.'?type=zareks&target_id='); ?>"+event.target.id+"&current_id="+current_id,true);
...
$(function() {
	...
	$('#file_upload').uploadifive({
		...
		'uploadScript' : '<?php echo base_url('admin/zareks_edit_event/'.$zareks_id); ?>',
