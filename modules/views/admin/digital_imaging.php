		...
		<table style="width:60%;">
			<tr>
				...
				<?php else: ?>
					...
					?>
					<td>
						<span id="digital_imaging<?php echo $i; ?>" style="color:#000; font-size:16px; cursor:pointer;">Edit</span>	
						...
						<script>
							document.querySelector('#u<?php echo $i; ?>').addEventListener('change', function(e) {
								uploadFiles('<?php echo base_url('admin/digital_imaging_cover_reupload/'.$photos_id.'?type='.$category.''); ?>',this.files,'uploading<?php echo $i; ?>','<?php echo base_url('admin/digital_imaging'); ?>');
							}, false);
					...
					<?php if($category == 'Retouching Restoration - Before and after'): ?>
						<div class="img_categ" onclick="location.href='<?php echo base_url('admin/category?type=before_after'); ?>'">
						<img class="img_size2" onclick="location.href='<?php echo base_url('admin/category?type=before_after'); ?>'" src="<?php echo base_url('uploads/digital_imaging/retouching/cover/'.$get['photos']) ?>" style="height:228px;" />
						<div style="float:left;font-size:16.66px;color:#5b5b5b;width:150px;">Retouching/Restoration</div>
					<?php elseif($category == 'Image Manipulation'): ?>
						<div class="img_categ" onclick="location.href='<?php echo base_url('admin/category?type=image_manipulation'); ?>'">
						<img class="img_size2" onclick="location.href='<?php echo base_url('admin/category?type=image_manipulation'); ?>'" src="<?php echo base_url('uploads/digital_imaging/image_manipulation/cover/'.$get['photos']) ?>" style="height:228px;" />
						<div style="float:left;font-size:16.66px;color:#5b5b5b;width:150px;">Image Manipulation</div>
					<?php else: ?>
					<?php endif; ?>
							<div style="overflow:hidden;padding:5px 0 10px 0;">
