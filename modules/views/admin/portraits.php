...
<?php get_header("admin/header",array('admin_bodycss','cc_box')); ?>
	...
	<div style="padding:20px 20px 0 20px; overflow:hidden">
		<table style="width:100%;">
			<tr>
				<?php	
					$query =  $this->QModel->sf('portrait_cover_photo');
				...
				<?php else: ?>
					...
					?>
					<td>
						<span id="portraits<?php echo $i; ?>" style="color:#000; font-size:16px; cursor:pointer;">Edit</span>	
						...
						<script>
							document.querySelector('#u<?php echo $i; ?>').addEventListener('change', function(e) {
								uploadFiles('<?php echo base_url('admin/portrait_cover_reupload/'.$photos_id.'?type='.$category.''); ?>',this.files,'uploading<?php echo $i; ?>','<?php echo base_url('admin/portraits'); ?>');
							}, false);
							
							var wed<?php echo $i; ?> = document.getElementById("portraits<?php echo $i; ?>");
							wed<?php echo $i; ?>.onclick = function(event) {
								document.uploading<?php echo $i; ?>.upload.click(event);
							};
						</script>
						<?php if($category == 'Family'): ?>
							<div class="img_categ" onclick="location.href='<?php echo base_url('admin/category?type=family'); ?>'">
							<img class="img_size2" onclick="location.href='<?php echo base_url('admin/category?type=family'); ?>'" src="<?php echo base_url('uploads/portraits/family/cover/'.$get['photos']) ?>" style="height:228px;" />
						<?php elseif($category == 'Individual'): ?>
							<div class="img_categ" onclick="location.href='<?php echo base_url('admin/category?type=individual'); ?>'">
							<img class="img_size2" onclick="location.href='<?php echo base_url('admin/category?type=individual'); ?>'" src="<?php echo base_url('uploads/portraits/individual/cover/'.$get['photos']) ?>" style="height:228px;" />
						<?php elseif($category == 'Children'): ?>
							<div class="img_categ" onclick="location.href='<?php echo base_url('admin/category?type=children'); ?>'">
							<img class="img_size2" onclick="location.href='<?php echo base_url('admin/category?type=children'); ?>'" src="<?php echo base_url('uploads/portraits/children/cover/'.$get['photos']) ?>" style="height:228px;" />
						<?php elseif($category == 'Pregnancy/Boudoir'): ?>
							<div class="img_categ" onclick="location.href='<?php echo base_url('admin/category?type=pregnancy_boudoir'); ?>'">
							<img class="img_size2" onclick="location.href='<?php echo base_url('admin/category?type=pregnancy_boudoir'); ?>'" src="<?php echo base_url('uploads/portraits/pregnancy_boudair/cover/'.$get['photos']) ?>" style="height:228px;" />
						<?php else: ?>
						<?php endif; ?>
