	...
	<div style="overflow:hidden; background:#ffffff; height:290px; width:31.5%; float:left; overflow:hidden; margin-left:9px; margin-bottom:9px;">
		<div style="text-align:center; width:auto; height:224px; overflow:hidden; margin:10px 10px 5px 10px; background:#e2e2e2;">
			<a href="<?php echo base_url('uploads/gallery/clients/'.$clients_id.'/original/'.$original_image); ?>" rel="lightbox[jnk]">
				...
				<tr>
					<td style="vertical-align:middle"><img src="<?php echo base_url("uploads/gallery/clients/".$clients_id."/semi-original/".$photos); ?>"/></td>
				</tr>
				</table>
			</a>
		</div>
		...
		<div style="height:44px; overflow:hidden;">
			<?php
				if($this->session->userdata('img_clients_id'.$clients_photos_id)):
			?>
				<input type="checkbox" name="selected[]" value="<?php echo $clients_photos_id; ?>" style="padding:10px; margin-left:10px; margin-top:10px;" checked />
			<?php else: ?>
				<input type="checkbox" name="selected[]" value="<?php echo $clients_photos_id; ?>" style="padding:10px; margin-left:10px; margin-top:10px;"/>
			<?php endif; ?>
			...
			<?php
				...
				if($this->QModel->c($q)):
			?>
				<img id="star<?php echo $clients_photos_id; ?>" src="<?php echo themes_url('images/icon_btnstar.png'); ?>" style="float:right; margin-right:5px; cursor:pointer;" onclick="markAsFavorite(<?php echo $clients_id; ?>,<?php echo $clients_photos_id; ?>,'clients');"/>
			<?php else: ?>
