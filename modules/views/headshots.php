						...
						<div style="float:left; width:48%; padding-right:0.2px;">
							<div style="width:97%; padding:20px;">
								<?php if($this->session->userdata('admin')): ?>
									<span id="headshots<?php echo $i; ?>" style="color:#fff; font-size:16px; cursor:pointer;">Edit</span>	
									...
									<script>
										document.querySelector('#u<?php echo $i; ?>').addEventListener('change', function(e) {
											uploadFiles('<?php echo base_url('admin/headshots_cover_reupload/'.$get['photos_id'].'?type='.$get['category'].''); ?>',this.files,'uploading<?php echo $i; ?>','<?php echo base_url('headshots'); ?>');
										}, false);
								...
								<?php else: ?>
								<?php endif; ?>
								<?php if($get['category'] == 'Adult'): ?>
								<a href="<?php echo base_url('headshots/category?type=adult'); ?>">
								<img class="img_size" src="<?php echo base_url('uploads/headshots/adult/cover/'.$get['photos']) ?>" style="height:400px;" />
								<?php elseif($get['category'] == 'Children'): ?>
								<a href="<?php echo base_url('headshots/category?type=children2'); ?>">
								<img class="img_size" src="<?php echo base_url('uploads/headshots/child/cover/'.$get['photos']) ?>" style="height:400px;" />
								<?php elseif($get['category'] == 'Business'): ?>
								<a href="<?php echo base_url('headshots/category?type=business'); ?>">
								<img class="img_size" src="<?php echo base_url('uploads/headshots/business/cover/'.$get['photos']) ?>" style="height:400px;" />
								<?php else: ?>
								<?php endif; ?>
								<br />
