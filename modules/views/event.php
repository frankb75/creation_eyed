						...
						<div style="float:left; width:48%; padding-right:0.2px;">
							<div style="width:97%; padding:20px;">
								<?php if($this->session->userdata('admin')): ?>
									<span id="events<?php echo $i; ?>" style="color:#fff; font-size:16px; cursor:pointer;">Edit</span>	
									...
									<script>
										document.querySelector('#u<?php echo $i; ?>').addEventListener('change', function(e) {
											uploadFiles('<?php echo base_url('admin/events_cover_reupload/'.$get['photos_id'].'?type='.$get['category'].''); ?>',this.files,'uploading<?php echo $i; ?>','<?php echo base_url('events'); ?>');
										}, false);
								...
								<a href="<?php echo base_url('events/category?type=weddings'); ?>">
								<img class="img_size" src="<?php echo base_url('uploads/events/weddings/cover/'.$get['photos']) ?>" style="height:400px;" />
								<?php elseif($get['category'] == 'Engagement'): ?>
								<a href="<?php echo base_url('events/category?type=engagement'); ?>">
								<img class="img_size" src="<?php echo base_url('uploads/events/engagement/cover/'.$get['photos']) ?>" style="height:400px;" />
								<?php elseif($get['category'] == 'Bar Mitzvahs'): ?>
								<a href="<?php echo base_url('events/category?type=bar_mitzvahs'); ?>">
								<img class="img_size" src="<?php echo base_url('uploads/events/bar_mitzvahs/cover/'.$get['photos']) ?>" style="height:400px;" />
								<?php elseif($get['category'] == 'Corporate'): ?>
								<a href="<?php echo base_url('events/category?type=corporate'); ?>">
								<img class="img_size" src="<?php echo base_url('uploads/events/corporate/cover/'.$get['photos']) ?>" style="height:400px;" />
								<?php else: ?>
								<?php endif; ?>
								<br />
